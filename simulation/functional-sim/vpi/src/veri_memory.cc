...
void veri_memory::fetch(unsigned int pc, SS_INST_TYPE& inst) {
  ...
  // bhdwiel: the initialization routine begins at 0x0 and is written into
  // mem_table -- not text_mem_table
  if (pc < 0x00010000) {
    inst = __UNCHK_MEMORY_ACCESS(SS_INST_TYPE, (pc & ~7));
  }

  else if ((pc < ld_text_base) || (pc > ld_text_base + ld_text_size)) {
    inst.a = 0;
    inst.b = 0;
  }
  else {
