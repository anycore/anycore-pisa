...
//`define CLKPERIOD 3.38  // 6x0.64
//`define CLKPERIOD 10.00  // 6x0.64
...
//* are freed every cycle, hence the IPC. Since
//* the freeing mechanism is a select tree, it can
//* be divided in to sub-trees and turned off when the
//* issue-queue size is scaled. Each sub-tree frees one
//* entry per cycle and even in the smallest configuration,
//* enough sub-trees should be on to free sufficient number
//* of entries per cycle. Hence, the number of sub-trees 
//* should be overprovisioned in case of a dynamic configuration.
`define IQ_FREEING_WIDTH             8
...
//`define DYNAMIC_WIDTH
`define DYNAMIC_SIZE

`ifdef DYNAMIC_CONFIG
  `ifdef DYNAMIC_WIDTH
    `define DYNAMIC_CONFIG_AND_WIDTH
  `endif
`endif
`ifdef DYNAMIC_CONFIG
  `ifdef DYNAMIC_SIZE
    `define DYNAMIC_CONFIG_AND_SIZE
  `endif
`endif
...
`ifdef INST_CACHE
  ...
  `define ICACHE_INSTS_IN_LINE 4*(2**`FETCH_WIDTH_LOG)  // 16
...
`define CLK_GATE_CELL_FG POSTICG_X2B_A12TR
`define CLK_GATE_CELL_UL POSTICG_X9B_A12TR
